...
elseif (isset($wp->query_vars['wcfm-products-manage']) && !empty($wp->query_vars['wcfm-products-manage']))
{
    ...
    if (wcfm_is_vendor())
    {
        $is_product_from_vendor = $WCFM
            ->wcfm_vendor_support
            ->wcfm_is_product_from_vendor($wp->query_vars['wcfm-products-manage']);
...
if (isset($wp->query_vars['wcfm-products-manage']) && !empty($wp->query_vars['wcfm-products-manage']))
{
    ...
    if ($product && !empty($product))
    {
        ...
        $sale_date_from = $product->get_date_on_sale_from('edit') && ($date = $product->get_date_on_sale_from('edit')
            ->getOffsetTimestamp()) ? date_i18n('Y-m-d', $date) : '';
        $sale_date_upto = $product->get_date_on_sale_to('edit') && ($date = $product->get_date_on_sale_to('edit')
            ->getOffsetTimestamp()) ? date_i18n('Y-m-d', $date) : '';

        $rich_editor = apply_filters('wcfm_is_allow_rich_editor', 'rich_editor');
        if (!$rich_editor && apply_filters('wcfm_is_allow_editor_newline_replace', true))
        {
            $breaks = apply_filters('wcfm_editor_newline_generators', array(
                "<br />",
                "<br>",
                "<br/>"
            ));
        ...
        // Download ptions
        $wcfm_downloadable_product_types = apply_filters('wcfm_downloadable_product_types', array(
            'simple',
            'subscription'
        ));
        ...
        if (in_array($product_type, $wcfm_variable_product_types))
        {
            ...
            if (!empty($variation_ids))
            {
                foreach ($variation_ids as $variation_id_key => $variation_id)
                {
                    ...
                    $variations[$variation_id_key]['enable'] = in_array($variation_data->get_status('edit') , array(
                        'publish',
                        false
                    ) , true) ? 'enable' : '';
                    ...
                    $variations[$variation_id_key]['sale_price_dates_from'] = $variation_data->get_date_on_sale_from('edit') && ($date = $variation_data->get_date_on_sale_from('edit')
                        ->getOffsetTimestamp()) ? date_i18n('Y-m-d', $date) : '';
                    $variations[$variation_id_key]['sale_price_dates_to'] = $variation_data->get_date_on_sale_to('edit') && ($date = $variation_data->get_date_on_sale_to('edit')
                        ->getOffsetTimestamp()) ? date_i18n('Y-m-d', $date) : '';
...
if ($product_shipping_class && !empty($product_shipping_class))
{
    ...
    {
        $variation_shipping_option_array[$product_shipping
            ->term_id] = $product_shipping->name;
        $shipping_option_array[$product_shipping
            ->term_id] = $product_shipping->name;
    }
}
...
$WCFM
    ->wcfm_fields
    ->wcfm_generate_form_field(apply_filters('wcfm_product_manage_fields_general', array(
    "product_type" => array(
        'type' => 'select',
...
$WCFM
    ->wcfm_fields
    ->wcfm_generate_form_field(apply_filters('wcfm_product_manage_fields_pricing', array(
    "product_url" => array(
    ...
    { ?>
						  	<?php if (apply_filters('wcfm_is_allow_product_category', true))
        {
            ...
            if ($product_categories)
            {
                $WCFM
                    ->library
                    ->generateTaxonomyHTML('product_cat', $product_categories, $categories);
            }
?>
									</select>
								<?php
        ...
        if (apply_filters('wcfm_is_allow_custom_taxonomy', true))
        {
            ...
            if (!empty($product_taxonomies))
            {
                ...
                {
                    if (!in_array($product_taxonomy->name, array(
                        'product_cat',
                        'product_tag',
                        'wcpv_product_vendors'
                    )) && apply_filters('wcfm_is_allow_product_taxonomy', true, $product_taxonomy->name) && apply_filters('wcfm_is_allow_taxonomy_' . $product_taxonomy->name, true))
                    {
...
?>
                                ...
                                if ($product_taxonomy_terms)
                                {
                                    $WCFM
                                        ->library
                                        ->generateTaxonomyHTML($product_taxonomy->name, $product_taxonomy_terms, $taxonomy_values_arr);
                                }
?>
														</select>
														<?php
    ...
    if ($wcfm_is_allow_tags = apply_filters('wcfm_is_allow_tags', true))
    {
        if (apply_filters('wcfm_is_tags_input', true))
        {
            $WCFM
                ->wcfm_fields
                ->wcfm_generate_form_field(apply_filters('wcfm_product_simple_fields_tag', array(
                "product_tags" => array(
        ...
        else
        {
            ...
            {
                $product_all_tags[$product_defined_tag
                    ->term_id] = $product_defined_tag->name;
            }
            $WCFM
                ->wcfm_fields
                ->wcfm_generate_form_field(apply_filters('wcfm_product_simple_fields_tag', array(
                ...
                foreach ($product_taxonomies as $product_taxonomy)
                {
                    ...
                    )) && apply_filters('wcfm_is_allow_product_taxonomy', true, $product_taxonomy->name) && apply_filters('wcfm_is_allow_taxonomy_' . $product_taxonomy->name, true))
                    {
                        if ($product_taxonomy->public && $product_taxonomy->show_ui && $product_taxonomy->meta_box_cb && !$product_taxonomy->hierarchical)
                        {
                            ...
                            {
                                // Fetching Saved Values
                                $taxonomy_values_arr = wp_get_post_terms($product_id, $product_taxonomy->name, array(
                                    "fields" => "names"
                                ));
                                $taxonomy_values = implode(',', $taxonomy_values_arr);
                                $WCFM
                                    ->wcfm_fields
                                    ->wcfm_generate_form_field(array(
...
if ($wcfm_is_allow_featured = apply_filters('wcfm_is_allow_featured', true))
{
    ...
    $WCFM
        ->wcfm_fields
        ->wcfm_generate_form_field(apply_filters('wcfm_product_manage_fields_images', array(
        "featured_img" => array(
            'type' => 'upload',
...
?>
            ...
            if ($product_categories)
            {
                $WCFM
                    ->library
                    ->generateTaxonomyHTML('product_cat', $product_categories, $categories, '', true);
            }
?>
										</ul>
									</div>
...
?>
                                ...
                                if ($product_taxonomy_terms)
                                {
                                    $WCFM
                                        ->library
                                        ->generateTaxonomyHTML($product_taxonomy->name, $product_taxonomy_terms, $taxonomy_values_arr, '', true, true);
                                }
?>
															</ul>
														</div>
